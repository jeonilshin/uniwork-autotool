...
export default function Statistics({ items, isAdmin, onClose }: StatisticsProps) {
  ...
  const summary = useMemo(() => {
    ...
    const profit = delivered.reduce((sum, item) => sum + ((item.sale - item.cost) * item.qty - (item.discount || 0)), 0);
  ...
  const chartData = useMemo(() => {
    ...
    filteredItems.forEach(item => {
      ...
      if (data[key]) {
        ...
        data[key].count += 1;
        if (item.status === 'delivered') data[key].profit += (item.sale - item.cost) * item.qty - (item.discount || 0);
      }
    });
